...
cudnn_conv2d_forward(torch::Tensor input, torch::Tensor weight,
  ...
  TORCH_CHECK(input.dim() == 4 && weight.dim() == 4, "expected NCHW and KCRS");
  ...
  auto output = torch::empty({n, k, out_h, out_w}, input.options());
  ...
  cudnn_conv2d_launcher(output.data_ptr(), input.data_ptr(), weight.data_ptr(),
                        ...
                        (int)dilation_w, (int)groups, dtype, (int)config,
                        stream);
  if (bias_ptr) {
    bias_add_launcher(output.data_ptr(), bias_ptr, n, k, out_h * out_w, dtype,
                      (int)config, stream);
  }
  return output;
}
...
static void register_cudnn_conv(pybind11::module &m) {
  m.def("cudnn_conv2d", &cudnn_conv2d_forward, "cuDNN Conv2d", py::arg("input"),
        ...
        py::arg("groups") = 1, py::arg("config") = 0);
}
