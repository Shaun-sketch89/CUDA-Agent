...
static torch::Tensor layernorm2d_forward(torch::Tensor input,
  ...
  TORCH_CHECK(input.dim() == 4, "NHWC expected");
  ...
  TORCH_CHECK(weight.numel() == input.size(3) && bias.numel() == input.size(3),
              "C mismatch");
...
static void register_layernorm2d(pybind11::module &m) {
  m.def("layernorm2d", &layernorm2d_forward, "Channel LayerNorm2d NHWC",
