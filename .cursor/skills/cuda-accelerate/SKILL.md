---
name: cuda-accelerate
description: >-
  Accelerate a PyTorch model with a high-performance CUDA C++ extension via
  implement → compile → verify → profile → iterate in agent_workdir. Use when
  optimizing CUDA kernels, writing model_new.py or kernels/, or running
  utils/compile, verification, or profiling against torch.compile.
---
...
**Workspace**: Run all work from `agent_workdir/` (relative to the repo root). Do not modify `utils/`, `binding.cpp`, or `binding_registry.h`.
...
# Optional: TensorRT single-layer LN baseline (do not modify these utils when optimizing kernels)
python3 -m utils.export_layernorm_onnx --mode decompose
python3 -m utils.trt_baseline --mode decompose
python3 -m utils.profiling --trt

# Sweep real NAFNet LayerNorm2d (C,H,W) per encoder/middle stage
python3 -m utils.nafnet_ln_sweep --preset width64_256
```
